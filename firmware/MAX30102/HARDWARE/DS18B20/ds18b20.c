...
{
  mode_output();                //输出模式
  GPIO_ResetBits(GPIOA,GPIO_Pin_8);              //拉低DQ引脚
  delay_us(600);         //延时480-960us
  GPIO_SetBits(GPIOA,GPIO_Pin_8);              //拉高DQ引脚  
  mode_input();
  while(GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_8));         //等待器件应答（器件拉低），约15-60us后
  while(!GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_8));        //应答脉冲出现后，等待器件拉高，约60-240us后
}
...
{
  mode_output();                    //输出模式
  GPIO_ResetBits(GPIOA,GPIO_Pin_8);                   //拉低引脚 
  delay_us(80);              //延时60-120us
  GPIO_SetBits(GPIOA,GPIO_Pin_8);                  //拉高引脚
  delay_us(2);
} 
